...
constexpr u8 AES_KEYS[][KEY_SIZE] = {
    ...
    {0xB8, 0x13, 0xC3, 0x5E, 0xC6, 0x44, 0x41, 0xE3, 0xDC, 0x3C, 0x16, 0xF5, 0xB4, 0x5E, 0x64, 0x84}, // New from PS3
    ...
    {0x60, 0x99, 0xF2, 0x81, 0x70, 0x56, 0x0E, 0x5F, 0x74, 0x7C, 0xB5, 0x20, 0xC0, 0xCD, 0xC2, 0x3C}, // New from PS3
    ...
    {0x7D, 0xF4, 0x92, 0x65, 0xE3, 0xFA, 0xD6, 0x78, 0xD6, 0xFE, 0x78, 0xAD, 0xBB, 0x3D, 0xFB, 0x63}, // New from PS3
    ...
    {0x0C, 0xFD, 0x67, 0x9A, 0xF9, 0xB4, 0x72, 0x4F, 0xD7, 0x8D, 0xD6, 0xE9, 0x96, 0x42, 0x28, 0x8B}, // 1.xx game eboot.bin
...
void kirkGenerateSHA1(u8 *data, u8 *out, u64 size) {
...
// KIRK command 7
void cmdDecryptAESKeystore() {
    std::puts("[KIRK    ] Decrypt AES (Keystore)");

    // Get pointers to source and dest buffers
    const auto srcBuffer = memory::getMemoryPointer(srcAddr);
    const auto dstBuffer = memory::getMemoryPointer(dstAddr);

    u32 mode, keyIdx, dataLength;
    std::memcpy(&mode, srcBuffer, sizeof(u32));
    std::memcpy(&keyIdx, &srcBuffer[0x0C], sizeof(u32));
    std::memcpy(&dataLength, &srcBuffer[0x10], sizeof(u32));

    std::printf("Mode: %u, key index: %u, data length: 0x%X\n", mode, keyIdx, dataLength);
    ...
    // Decrypt data area
    u8 data[dataLength];
    std::memcpy(data, &srcBuffer[0x14], dataLength);
...
void cmdGenerateSHA1() {
    std::puts("[KIRK    ] Generate SHA-1");
    ...
    u32 dataLength;
    std::memcpy(&dataLength, srcBuffer, sizeof(u32));

    std::printf("Data length: 0x%X\n", dataLength);

    kirkGenerateSHA1(&srcBuffer[4], dstBuffer, dataLength);
}
...
void doCommand() {
    switch ((KIRKCommand)cmd) {
        case KIRKCommand::DECRYPT_PRIVATE:
            cmdDecryptPrivate();
            break;
        case KIRKCommand::DECRYPT_AES_KEYSTORE:
            cmdDecryptAESKeystore();
            break;
...
void write(u32 addr, u32 data) {
    switch ((KIRKReg)addr) {
        ...
        case KIRKReg::PRVSTS:
            ...
            intc::clearIRQ(intc::InterruptSource::KIRK); // ??? Maybe??
            break;
        case KIRKReg::SRC:
